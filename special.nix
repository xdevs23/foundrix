  ...
  namespaced = curPos: options:
    let
      ...
      baseName =
        ...
        then builtins.head (builtins.match "(.*)\.nix" fileName)
        else fileName;
      ...
      mkNested = parts: value:
        if parts == [] then value
        else { ${builtins.head parts} = mkNested (builtins.tail parts) value; };
    in mkNested allParts options;
}