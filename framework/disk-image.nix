{ namespaced, namespacedCfg, lib, config, pkgs, modulesPath, ... }: {
  imports = [
  ...
  options = namespaced __curPos {
    ...
    efi.enable = (lib.mkEnableOption "EFI suppport") // { default = pkgs.hostPlatform.is64bit; };
    ...
    partitionIds = lib.mkOption {
      ...
      default = {
        esp = "10-esp";
        store-verity = "20-store-verity";
        empty-store-verity = "25-store-verity";
        store = "30-store";
        ...
        partitions = {
          ...
          ${partitionIds.store} = {
            ...
            repartConfig = rec {
              Type = "root";
              SplitName = "store";
              Label = let version = config.system.image.version or null; in
